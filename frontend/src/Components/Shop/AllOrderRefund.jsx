...
const AllOrderRefund = () => {
    ...
    const eligibleOrder = shopOrders && shopOrders.filter((order) => {
        return order.status === "Processing refund" || order.status === "Refund successful"
    }
    )
 
    const row = []
    eligibleOrder && eligibleOrder.forEach((order) => {
        row.push({
            id: order?._id,
